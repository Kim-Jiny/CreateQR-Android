package com.jiny.createqr.presentation.create
...
import com.jiny.createqr.domain.usecase.GetQRTypeListUseCase
...
class CreateQRViewModel @Inject constructor(
    ...
    fun saveQRToApp(title: String? = null) {
        viewModelScope.launch {
            ...
            val selectedType = _selectedType.value?.type ?: CreateType.URL

            // URL 타입인 경우 실제 URL인지 확인하여 TEXT와 구분
            val type = if (selectedType == CreateType.URL) {
                if (isUrl(data)) CreateType.URL else CreateType.TEXT
            } else {
                selectedType
            }
